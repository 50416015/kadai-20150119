...
public class ReservationControl {
    ...
    public String loginLogout( MainFrame frame){
        ...
        if ( flagLogin){ //ログアウトを行う処理
            ...
            //ユーザーIDとパスワードが入力された場合の処理
            //ユーザーIDは他の機能のときに使用するのでメンバー変数に代入
    ...
    public String confirmReservation(MainFrame frame){
        ...
        try {
            ...
            while(rs.next()){
                ...
                String start = rs.getString("start_time");
                String end = rs.getString("end_time");
                res += facility_name +" "+ rdate + " " + start + " -- " + end + "\n";
                exist = true;
            }
            if ( !exist){ //予約が1つも存在しない場合の処理
                res = "予約はありません";
...
public String CancelReservation(MainFrame frame){
    ...
    if ( flagLogin){ // ログインしていた場合
        //予約キャンセル画面作成
        CancelReservation rd = new CancelReservation(frame);
                    ...
                    try {
                        ...
                        ResultSet rs = mysql.selectReservation(rdate, facility);
                          ...
                          while(rs.next()){
                                ...
                                String start = rs.getString("start_time");
                                String end = rs.getString("end_time");
                                if ( (start.compareTo(st)==0 && end.compareTo(et)==0)) {        //レコードの開始時刻==新規の開始時刻　AND　新規の開始時刻==レコードの終了時刻
                                         // 重複有りの場合に ng をtrueに設定
                                    ng = true; break;
                                }
                          }
